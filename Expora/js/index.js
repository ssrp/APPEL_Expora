...
// This variable will be used to switch between the Output Section panels.
var queryoutput = document.getElementsByClassName("query_result_options");
...
// Creating the google charts output
function showOutput()
{
...
function start(){
	...
	queryoutput[1].style.display = "none";
	queryoutput[2].style.display = "none";
	loadTextFields();
	loadMetadataFields();
	buildHistory();
	buildShared();
...
var currQueryOutput = 0;

// Used to go to any other option!
function gotoQueryOutput(N)
{
			queryoutput[currQueryOutput].style.display = "none";
			queryoutput[N].style.display = "block";
			currQueryOutput = N;	
}
